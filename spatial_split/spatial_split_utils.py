...
def lon_to_global_easting(lon,origin = (0.0,0.0)):
...
def lat_to_global_northing(lat,origin = (0.0,0.0)):
    ...
    for i in range(len(lat)):
        ...
        if lat_ofs < 0:
            ...
            global_northings_km[i] = WGS84_MERIDIAN_CIRCUMFERENCE - global_northings_km[i]
    return global_northings_km

def assign_block_ids(lon,lat,east_west_bin_km,north_south_bin_km,origin=(0.0,0.0)):
    ...
    (easting_bin_list,northing_bin_list) = coords_to_bin(easting_list_km,northing_list_km,east_west_bin_km,north_south_bin_km)
    
    block_id_list = []
    for i in range(len(easting_bin_list)):
        block_id_list.append('e'+str(easting_bin_list[i]).zfill(4)+'n'+str(northing_bin_list[i]).zfill(4))
